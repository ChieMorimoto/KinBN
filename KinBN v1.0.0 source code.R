KinBN <- function(ped.max = 25, range = 0){
  ...
  makeLinkage_Hypo <- function(File.result){
    ...
    combobox_Linkage.locus1 <- combobox_Linkage.locus2 <- entry_Linkage.RR <- list()
    linkage_number <- 1

    outframe_Linkage.setting <- tkframe(frame_Linkage)
    frame_Linkage.radiobutton <- tkframe(outframe_Linkage.setting)
    frame_Linkage.add.vanish <- tkframe(outframe_Linkage.setting)
    ...
    cb_default.change <- function(x){
      if (x == 1){
        tkpack(frame_Linkage.add.vanish, anchor="w", padx=40, pady=5)
        ...
        tkconfigure(button_Linkage.vanish, state="disable")
      }
      else if(x == 0){
        tkpack(frame_Linkage.add.vanish, anchor="w", padx=40, pady=5)
        tkpack(frame_Linkage.label, anchor="w", padx=40, pady=5)
        ...
        tkconfigure(button_Linkage.vanish, state="normal")
      }
      else{
        tkpack.forget(frame_Linkage.add.vanish)
        tkpack.forget(frame_Linkage.label)
        tkpack.forget(frame_Linkage.select)
      }
    }
    
    button_Linkage.add <- tkbutton(frame_Linkage.add.vanish,text="Add",font="our_font",width=10,state="disable",
                                   command = function(){
                                     if (1 <= linkage_number && linkage_number < pair.max){
                                       linkage_number <<- linkage_number + 1
                                       tkpack(frame_Linkage.select.input[[linkage_number]],anchor="w", pady="5")
                                     }
                                                       }
                                   )
    button_Linkage.vanish <- tkbutton(frame_Linkage.add.vanish,text="Delete",font="our_font",width=10,state="disable",
                                      command = function(){
                                        if (1 < linkage_number && linkage_number <= pair.max){
    ...
    if (File.result$locus_set_name != "-"){
      ...
      useDefault <- tclVar("0")
      radiobutton_on <- tkradiobutton(frame_Linkage.radiobutton, anchor="w", width=40, font="our_font", text="Manual setting", variable=useDefault, value=0, command=function(){checkbutton_default.change(as.integer(tclvalue(useDefault)))})
      radiobutton_none <- tkradiobutton(frame_Linkage.radiobutton, anchor="w", width=40, font="our_font", text="No linkage", variable=useDefault, value=-1, command=function(){checkbutton_default.change(as.integer(tclvalue(useDefault)))})
      for (i in 1:pair.max){
        tkconfigure(combobox_Linkage.locus1[[i]], state="readonly")
        tkconfigure(combobox_Linkage.locus2[[i]], state="readonly")
        tkconfigure(entry_Linkage.RR[[i]], state="normal")
    ...
    tkpack(button_Linkage.vanish, padx=10, side="left")
  ...
  if (length(non_founder) >= 1){ 
    ...
    for(m in 1:(dim(alleletype)[4])){  
      for (k in 1:length(al)){
        ...
        known_gt <- apply(alleletype[,k,DNAtype,m],2,function(x){return(paste(min(x),"-",max(x),sep=""))})
  ...
  if (length(non_founder) >= 1){ 
    ...
    for(m in 1:dim(alleletype)[4]){
      for (l in 1:nrow(linkage.loci)){
        ...
        known_gt_L1 <- apply(alleletype[,linkage.loci[l,1],DNAtype,m],2,function(x){return(paste(min(x),"-",max(x),sep=""))})
        known_gt_L2 <- apply(alleletype[,linkage.loci[l,2],DNAtype,m],2,function(x){return(paste(min(x),"-",max(x),sep=""))})
...
Calculate <- function(Hypo.result){
    ...
    if(length(which(check.result==1))==1){      
      ...
      for (j in 1:2){ 
        if (is.na(Hypo.result$RR[1])){
          LR.step[[j]] <- array(NA, dim=c(No.known.ref - 1, length(Hypo.result$locus_set)))
        }else{
          LR.step[[j]] <- list()
          LR.step[[j]][[1]] <- array(NA, dim=c(No.known.ref - 1, length(Hypo.result$locus_set) - 2 * length(Hypo.result$RR)))
          LR.step[[j]][[2]] <- array(NA, dim=c(No.known.ref - 1, length(Hypo.result$RR)))
        }
      }

      for (i in 1:(No.known.ref - 1)){
        for (j in 1:2){
  ...
  Simulate <- function(Hypo.result){
    ...
    if(length(which(check.result==1))==1){
      ...
      for (j in 1:2){
        for (i in 1:(sum(Hypo.result$df[[j]]$affected) - 1)){
          for(k in 1:2){
            target_number[j] <- which(df.original[[j]][[i]]$DNAtype == i)
            if (is.na(Hypo.result$RR[1])){
              LR.step[[k]][[j]] <- array(NA, dim=c(Hypo.result$N, length(Hypo.result$locus_set), sum(Hypo.result$df[[j]]$affected) - 1))
              calc.info <- c(LR.info[[k]], list(df = df.original[[j]][[i]], target_number = target_number[j]))
              LR.step[[k]][[j]][,,i] <- PES_function(calc.info)
            }else{
              LR.step[[k]][[j]] <- list()
              LR.step[[k]][[j]][[1]] <- array(NA, dim=c(Hypo.result$N, length(Hypo.result$locus_set) - 2 * length(Hypo.result$RR), sum(Hypo.result$df[[j]]$affected) - 1))
              LR.step[[k]][[j]][[2]] <- array(NA, dim=c(Hypo.result$N, length(Hypo.result$RR), sum(Hypo.result$df[[j]]$affected) - 1))
              calc.info <- c(LR.info[[k]], list(df = df.original[[j]][[i]], target_number = target_number[j]))
              temp.result <- PES_function_linkage(calc.info)
