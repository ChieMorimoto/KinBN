makeoldHypo <- function(envProj, envGUI, modeStateVar){
  ...
  ref.num <- get("ref.num", pos = envProj)
  ...
  var_ref <-  df[[1]]$names[which(df[[1]]$affected==1)]
  ...
  ped.number <- c(length(df[[1]]$id), length(df[[2]]$id))

  for (j in 1:2){
    for (i in 1:ped.number[j]){
      tclvalue(sex.var[[j]][[i]]) <- sex.val_to_var(df[[j]]$sex[i])
      tclvalue(pat.var[[j]][[i]]) <- df[[j]]$names[df[[j]]$pat[i]]
      tclvalue(mat.var[[j]][[i]]) <- df[[j]]$names[df[[j]]$mat[i]]
      if(df[[j]]$pat[i]==0){
        tclvalue(founder.var[[j]][[i]]) <- 1
      }
    }
  }
  ...
  for (j in 1:2){
    for (i in 1:ped.max){
      ...
      combobox_Hypo.sex[[j]][[i]] <- ttkcombobox(frame_Hypo.select.input[[j]][[i]], values=var_sex, textvariable=sex.var[[j]][[i]], cursor="hand2", width=6, state="readonly")
      ...
      if(tclvalue(founder.var[[j]][[i]]) == 1){
        tkconfigure(combobox_Hypo.pat[[j]][[i]], state="disable")
        tkconfigure(combobox_Hypo.mat[[j]][[i]], state="disable")
      }
    }
  }
